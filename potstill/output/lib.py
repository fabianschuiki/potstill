...
def make_lib(timing, layout):
	opcond = "vdd=%g,temp=%g" % (timing.macro.vdd, timing.macro.temp)
	figures = timing.figures[opcond]

	wr = LibWriter()
	wr.comment(
		"Standard Cell Based Memory",
		"%d words, %d bits @ at %gV, %g°C" % (timing.macro.num_words, timing.macro.num_bits, timing.macro.vdd, timing.macro.temp),
		None,
	...
	wr.simple("nom_temperature", "%.0f" % timing.macro.temp)
	wr.simple("nom_voltage", "%.2f" % timing.macro.vdd)
	wr.skip()
	...
	wr.pushgrp("type", "A%d" % timing.macro.num_addr)
	wr.simple("base_type", "array")
	wr.simple("data_type", "bit")
	wr.simple("bit_width", str(timing.macro.num_addr))
	wr.simple("bit_from", str(timing.macro.num_addr-1))
	wr.simple("bit_to", "0")
	wr.simple("downto", "true")
	wr.popgrp()
	wr.pushgrp("type", "D%d" % timing.macro.num_bits)
	wr.simple("base_type", "array")
	wr.simple("data_type", "bit")
	wr.simple("bit_width", str(timing.macro.num_bits))
	wr.simple("bit_from", str(timing.macro.num_bits-1))
	wr.simple("bit_to", "0")
	wr.simple("downto", "true")
	wr.popgrp()
	wr.skip()
	...
	wr.pushgrp("cell", timing.macro.name)
	...
	for (name, bus_type, char_name) in [
		("RA", "A%d" % timing.macro.num_addr, "RA0"),
		("WA", "A%d" % timing.macro.num_addr, "WA0"),
		("WD", "D%d" % timing.macro.num_bits, "WD0")
	]:
